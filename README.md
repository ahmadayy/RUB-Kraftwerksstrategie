...
it, and answered the question with real, reproducible numbers.*
...
│                             #  (raw & processed hourly data are regenerated by data_loader.py)
│
...
Energy-Charts data source is token-free).
...
- **Calibration:** my modelled 2025 mean price is **€89.72/MWh** vs the realised market **€89.33/MWh**
  → **0.4% error** (`results/calibration_2025_validation.csv`).
...
- **Single source of truth:** every model number reported elsewhere is read directly from
  `results/comparison_table.csv`, so the figures, the report, and the slides stay identical.

---

## ⚠️ Known limitations

I state these openly:
